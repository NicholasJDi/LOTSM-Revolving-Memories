...
func _process(_delta: float) -> void:
	if not Global.Player.invincible:
		if Input.is_action_pressed("swap_powers") and not get_tree().paused:
			if Input.is_action_just_pressed("swap_powers+1"):
				Global.Player.current_power_set = 1
			...
			if Input.is_action_just_pressed("swap_powers+2"):
				Global.Player.current_power_set = 2
			...
			if Input.is_action_just_pressed("swap_powers+3"):
				Global.Player.current_power_set = 3
			...
			if Input.is_action_just_pressed("swap_powers+4"):
				Global.Player.current_power_set = 4
			...
			if Input.is_action_just_pressed("swap_powers+5"):
				Global.Player.current_power_set = 5
			...
			if Input.is_action_just_pressed("swap_powers+6"):
				Global.Player.current_power_set = 6
