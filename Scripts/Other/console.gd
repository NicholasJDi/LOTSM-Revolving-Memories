...
func _ready() -> void:
	ConsoleWindow._Print.connect(Print)
	add_message(ConsoleWindow.type, ConsoleWindow.level, ConsoleWindow.text)
	Type = ConsoleWindow.Types
	last_commands = ConsoleWindow.Last_Commands
	username = ConsoleWindow.Username
...
func use_command(text):
	...
	ConsoleWindow.Last_Commands = last_commands
	if text == "/help" or text == "/h":
		...
		if text == "/name" or text == "/name " or text == "/name help" or  text == "/name h":
			...
			if tempusername.count(tempusername, 0, 14):
				...
				ConsoleWindow.Username = text.replace("/name ", "")
				input_label.text = "[" + username + "]"
